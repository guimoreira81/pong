...
window.addEventListener("keydown", (event) => {
    ...
    if (playing){
        if (mode != "botXbot" && event.key == "w" && player1.position.y+player1.size.y/2 < game.unitY*50){
            player1.position = player1.position.add(new Vector2(0, playerSpeed))
        }
        if (mode!="botXbot" && event.key == "s" && player1.position.y-player1.size.y/2 > -game.unitY*50){
            player1.position = player1.position.add(new Vector2(0, -playerSpeed))
        }
        
        if (mode == "playerXplayer" && event.key == "ArrowUp" && player2.position.y+player2.size.y/2 < game.unitY*50){
            player2.position = player2.position.add(new Vector2(0, playerSpeed))
        }
        if (mode == "playerXplayer" && event.key == "ArrowDown" && player2.position.y-player2.size.y/2 > -game.unitY*50){
            player2.position = player2.position.add(new Vector2(0, -playerSpeed))
        }
    }
})
